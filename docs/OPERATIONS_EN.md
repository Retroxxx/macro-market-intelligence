...
NiuOne requires a large language model for the complete trading-decision workflow. The daily U.S. institutional ratings report does not call a model: it uses Financial Modeling Prep (FMP) structured rating, price-target, and quote data, then applies local buy-bias filtering, deduplication, institutional clustering, and ranking. A-share auction, midday, and close summaries share one market-summary model with the overnight U.S. market summary and no longer read the legacy Grok settings. News prechecks for A-share candidates and dragon-tiger signals use iWencai only. Trading decisions after stock selection can use a compatible model, with DeepSeek recommended.
...
| A-share and overnight U.S. market summaries | `A_SHARE_MODEL_SUMMARY_BASE_URL`, `A_SHARE_MODEL_SUMMARY_API_KEY`, `A_SHARE_MODEL_SUMMARY_MODEL`, `A_SHARE_MODEL_SUMMARY_STREAM_MODE`, `A_SHARE_MODEL_SUMMARY_REASONING_EFFORT`, `A_SHARE_MODEL_SUMMARY_MAX_TOKENS` |
