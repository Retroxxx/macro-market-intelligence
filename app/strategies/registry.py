...
def default_trade_discipline_text(
    *,
    ...
    opening_count_rule = (
        ...
        "只硬限制同时最多5只持仓，满仓时按可审计优先级执行严格先卖后买换仓。"
        if niuone_enabled
        else (
    ...
    registered_position_rule = (
        "- 牛牛战法动态风险预算、总仓/主题敞口、最多5只持仓、领涨30%/转强25%/启动15%/试仓10%绝对上限，以及主升早期10%与确认主升20%的本地确定性分级加仓上限，都是执行层硬限制，不是参考值。"
        if niuone_enabled
        else
