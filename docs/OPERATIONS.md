...
| `DASHBOARD_NIUONE_FORWARD_COHORT_START` | `2026-08-31` | 严格前向队列纳入首次 BUY 的起始交易日 | 后续 Cron 周期读取；改值后必须建立新协议锁 |
| `DASHBOARD_EXIT_FEEDBACK_AUTO_TUNE_ENABLED` | `0` | 是否启用受约束的 5 日卖后自动调参 | 下一轮盘后复盘读取 |
| `DASHBOARD_EXIT_FEEDBACK_MIN_SAMPLES` | `30` | 调参最少完整样本，允许 20～500 | 下一轮盘后复盘读取 |
| `DASHBOARD_EXIT_FEEDBACK_MIN_MONTHS` | `3` | 调参最少覆盖月份，允许 2～12 | 下一轮盘后复盘读取 |
| `DASHBOARD_EXIT_FEEDBACK_COOLDOWN_SAMPLES` | `10` | 每轮调参要求的新增样本，允许 5～100 | 下一轮盘后复盘读取 |
