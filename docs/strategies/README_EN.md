...
Strict-forward v34 and backtest v35 remove NiuOne morning/afternoon, per-decision, and per-day opening-count limits while fixing the book at five holdings. Full-book replacement uses the same deterministic priority and full T+1 sellability check in Practice and replay; only a strictly higher candidate may trigger SELL-before-BUY. Risk budgets, theme capacity, limit-up rejection, and structural stops are unchanged. The new forward cohort begins on `2026-08-19`; older locks, reports, and backtests must not be pooled.

Strict-forward v35 and backtest v36 add same-name score-ladder scaling. Every filled BUY records its signal score, the holding-period high BUY score, and the fill count; another signal under the same strategy may add only when its score sets a strict new high. Ties, lower scores, and missing baselines fail closed. Probe cannot add on its entry day or average down; mature paths still require Markup, strong-leader status, and the 2%–12% profit window. Stage upgrades, post-trim wave re-entry, and every single-name/theme/portfolio risk cap remain independent. The not-yet-started forward cohort remains dated `2026-08-19`.

Strict-forward v36 additionally makes NiuOne opening counts independent of the current-market summary/evaluation. Holding-count, per-decision BUY-count, and pause fields in the shared market context govern non-NiuOne strategies only; NiuOne always uses the five-name ceiling, strict full-book replacement priority, and the existing per-name/theme/portfolio risk budgets. A candidate's own compound market hard stop and the independent daily-loss budget can still block BUY. Admin backtest already uses the same capacity semantics and remains v36, while the default forward cohort remains dated `2026-08-19`.

Strict-forward v37 treats failed, timed-out, unchecked, pending, or unavailable news prechecks as neutral with zero decision weight. Such records remain diagnostic state but are omitted from model news evidence and cannot lower score, priority, or sizing, or justify no entry, HOLD, or SELL. Completed positive, negative, and neutral judgments still affect candidate risk normally. Admin backtest does not consume this model-news weighting and remains v36; the default forward cohort remains dated `2026-08-19`.

Strict-forward v38 and backtest v37 remove the fixed two-Probe daily candidate cap and the fixed two-position same-sector/same-theme cap from NiuOne. The five-position ceiling, single-name, theme-risk, theme-exposure, portfolio-risk, total-exposure, cash, limit-up, and T+1 controls remain; Sector Tide is unchanged. The new forward cohort starts on `2026-08-21`, and older locks, reports, and backtests must not be pooled.

Strict-forward v39 removes the remaining ambiguity in the shared market-context prompt: `max_open_positions` and the pre-lunch reserve note apply only to non-NiuOne strategies. When evaluating a NiuOne BUY or HOLD, the model must compare the current NiuOne book only with the five-position ceiling. The NiuOne algorithm and administrator backtest remain on v37; the new forward cohort starts on `2026-08-24`, and pre-fix and post-fix decision evidence must not be pooled.

Strict-forward v40 raises the Probe absolute cap to 10% and raises both the rotation Probe per-trade NAV-risk and theme-risk ceiling to 1%. A holding that is in cross-session Markup leadership with 2%–12% unrealized profit receives deterministic local 10%/20% staged scale-in actions instead of depending on the model to emit ADD. An explicit model SELL remains exit-first, and the executor still rechecks single-name, theme, portfolio, total-exposure, cash, and T+1 boundaries. Strict-forward advances to `niuone-strict-forward-v40`, administrator backtests advance to `niuone-backtest-v38`, the default cohort remains `2026-08-24`, and older evidence must not be pooled.

Strict-forward v41 adds a holding fast cycle that is disabled by default. It narrows only the incremental scoring universe to current positions and still uses the complete cycle's active scorers, candidate selection, unified market summary, model policy, exit-first ordering, and execution controls. BUY is add-only for a position that still exists at execution and cannot discover, open, or reopen a symbol. The durable origin is `holding_fast`, and the switch plus bounded 60–900-second interval enter the protocol fingerprint. Administrator daily-bar backtests remain on `niuone-backtest-v38`, strict-forward advances to `niuone-strict-forward-v41`, the new default cohort starts on `2026-08-27`, and older evidence must not be pooled.

Strict-forward v42 and backtest v39 align NiuOne's concurrent-holding ceiling with `DASHBOARD_MAX_OPEN_POSITIONS`. Practice prompts, free-slot calculation, full-book replacement, and execution rechecks use the startup value; backtests snapshot it at submission, and strict-forward records it in the protocol identity. Pre-lunch and market-summary dynamic counts remain non-NiuOne-only, while every other risk budget is unchanged. The default forward cohort remains `2026-08-27`; older evidence and backtest results must not be pooled.

Strict-forward v43 and backtest v40 share staged soft-exit and full-book replacement semantics. Non-structural no-progress, score, theme/sector weakening, and ordinary giveback exits trim 50% first, close the runner only after a later-session confirmation, and allow a 4–5 score to veto the first session; hard stops remain immediate, and replacement requires a three-point priority advantage. Practice also persists 1/3/5/10-session post-exit outcomes and applies a five-session price, volume, and original-theme reopening gate after a staged full close. The new forward cohort starts on `2026-08-28`; older evidence and backtest results must not be pooled.
Strict-forward v44 connects the five-session outcomes to an opt-in bounded feedback version. Once enabled, it needs at least 30 complete observations spanning three months and can move only one declared grid step per 10 new observations. Only soft-exit confirmations/trim ratio, the 3–5 point replacement margin, and post-exit volume/activity gates are tunable; structural stops and every portfolio-risk boundary remain frozen. Fills bind the policy version, and materially worse version-tagged outcomes restore the prior parameters automatically. Backtest v40 retains fixed defaults; the new forward cohort starts on `2026-08-31`.
Strict-forward v45 upgrades the feedback algorithm to v2. Completed outcomes cannot be downgraded; exit returns use the actual fill, replacement evidence requires an executed BUY, and allowed or blocked re-entry decisions retain direct five-session shadow results. Only the latest 120 valid observations are used, correlated same-security/same-day fills are clustered, and capital-weighted 90% confidence intervals must clear a minimum economic effect. Otherwise a hold checkpoint is recorded without creating a policy version. Runtime state reconciles against the active SQLite version, while structural stops and every portfolio-risk boundary remain excluded from tuning.
Strict-forward v46 enables this bounded feedback loop by default. Without an explicit setting, the next post-close review begins learning or evaluation and moves one grid step only after every gate passes. Setting the switch to `0` still opts out; sample gates, bounded grids, automatic rollback, and frozen risk controls are unchanged.
