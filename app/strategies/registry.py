...
def default_trade_discipline_text(
    *,
    ...
    position_rule = (
        ...
        "- 牛牛战法由动态风险预算决定仓位：进攻/轮动/修复/防守的确认路径单笔风险预算分别为权益1.50%/1.00%/0.60%/0.30%，日线V型试仓仅0.35%/0.30%/0.25%/0.15%；防守允许开仓，复合风险硬停止才禁止新仓。试仓须满足题材至少6只强势股或酝酿连续3个交易日，单票绝对上限6.25%。试仓/启动持仓浮盈处于2%～12%、仍在主升且个股保持强势领涨时，启动主线跨日延续先向10%上限加一次，主线确认后再向20%上限加一次。策略内组合未实现止损风险≤4.50%/3.00%/1.80%/0.90%，总仓≤70%/55%/35%/20%。领涨/转强/启动的30%/25%/15%仅是新开路径绝对天花板；不设固定同板块或同题材持仓只数上限，集中风险继续受主题风险与主题敞口预算约束，同时最多5只。"
        if niuone_enabled
        else
