...
| `DASHBOARD_NIUONE_FORWARD_COHORT_START` | `2026-08-31` | First-entry date admitted to the strict-forward cohort | Read by a subsequent Cron cycle; changing it requires a new protocol lock |
| `DASHBOARD_EXIT_FEEDBACK_AUTO_TUNE_ENABLED` | `0` | Enable bounded five-session post-exit tuning | Read by the next post-close review |
| `DASHBOARD_EXIT_FEEDBACK_MIN_SAMPLES` | `30` | Minimum complete observations, from 20 through 500 | Read by the next post-close review |
...
| `DASHBOARD_EXIT_FEEDBACK_COOLDOWN_SAMPLES` | `10` | New observations required per evaluation, from 5 through 100 | Read by the next post-close review |
