...
- **信息聚合与模型分析**：通过 NewsNow 聚合财联社与金十实时快讯，并整理 A 股竞价、午盘、盘后报告，以及隔夜美股、机构评级、X 关注源和问财龙虎榜。兼容的大模型服务可用于消息检索、摘要和结构化分析。
...
| `/realtime-news` | NewsNow 聚合的可选财经新闻来源 | 无需 API Key；Compose 默认随附内置 NewsNow，管理页可搜索并多选财经商业来源 |
...
项目提供单一牛牛1号镜像和 Compose 编排。Compose 会启动 dashboard、定时调度器、推特关注源守护进程以及官方 NewsNow 实例。牛牛1号配置、数据库、日志和任务输出保存在 `niuone-data` volume，NewsNow 数据单独保存在 `newsnow-data` volume。
